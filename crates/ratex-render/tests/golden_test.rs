...
fn run_golden_suite(
    ...
    for (i, expr) in lines.iter().enumerate() {
        ...
        let test_ch = if test_buf.len() == (info.width * info.height * 4) as usize {
            4
        } else {
            3
        };
...
fn cjk_smoke_non_blank_rendering() {
    ...
    for ch in emoji_chars {
        ...
        if ratio > 0.001 {
            ...
            eprintln!(
                "CJK smoke '\\text{{{ch}}}': {:.1}% ink coverage",
                ratio * 100.0
            );
        } else {
...
mod macos_font_cjk_cmap {
    ...
    fn apple_gothic_missing_hanzi_is_glyph_zero() {
        let bytes = std::fs::read("/System/Library/Fonts/Supplemental/AppleGothic.ttf")
            .expect("AppleGothic");
