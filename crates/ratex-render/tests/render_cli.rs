...
#[test]
fn parse_error_exits_one_and_reports_failed_png() {
    let output_dir = TempDir::new("render-cli-parse-error");
    ...
    assert!(stdout.contains("Processed 1 formula(s), wrote 0 PNG(s), failed 1."));
    assert!(stderr.contains("ERR    1"));
    assert!(!output_dir.path().join("0001.png").exists());
}

#[test]
fn invalid_color_exits_two() {
    let output_dir = TempDir::new("render-cli-invalid-color");
